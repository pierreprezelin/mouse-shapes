...
const specs: SpecItem[] = [
  ...
  {
    label: "Hand Compatibility",
    key: "hand_compatibility",
    map: HAND_COMPATIBILITY,
  },
...
<template>
  <section>
    <div
      class="relative mx-auto max-h-[calc(100vh-var(--ui-header-height)-72px)] w-fit max-w-full overflow-x-auto overflow-y-auto rounded-lg border border-zinc-200 dark:border-zinc-700"
    >
        ...
        <thead class="relative z-20">
          <tr class="border-b border-zinc-200 dark:border-zinc-700">
            ...
            <th
              ...
              :key="model.id"
              class="bg-default sticky top-0 z-20 w-52 min-w-56 p-4 text-center font-semibold"
            ...
            >
              ...
              />
              <span class="text-muted mt-2.5 block text-sm">
                {{ model.brand }}
              </span>
              {{ model.name }}
            </th>
          ...
          <tr v-for="spec in specs" :key="spec.label">
            ...
            >
              ...
              <template v-else-if="spec.map">
                {{
                  model[spec.key] !== null
                    ? spec.map[model[spec.key] as any]
                    : "-"
                }}
              </template>
              <template v-else>
